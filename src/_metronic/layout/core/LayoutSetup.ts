...
export class LayoutSetup {
  ...
  private static initLayout(config: ILayout): void {
    ...
    document.body.style.backgroundImage =`url(${toAbsoluteUrl('/media/patterns/primary-gradient.png')})`
    /*axios.get(`${env.REACT_APP_BACKEND_API_PORT_URL}/settings/info/headerImage`)
    .then(response => {

      document.body.style.backgroundImage = `url(data:image/png;base64,${response.data})`
    })
    .catch(error => {
        console.log(error);
    });*/
